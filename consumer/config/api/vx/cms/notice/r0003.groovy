...
class r0003 {
    ...
    def output(output) {
        ...
        for (def contentItem : output.getContent()) {
            ...
            data['flagName'] = ApplicationContextHolder.getBean(DictionaryService.class).readOneByDictionaryTypeCodesCache(['dictionaryTypeCodes': 'noticeFlag,flag', 'dictionaryCode': contentItem['flag_code'], 'object_id': 'name'])

            data['stateName'] = ApplicationContextHolder.getBean(DictionaryService.class).readOneByDictionaryTypeCodesCache(['dictionaryTypeCodes': 'noticeState,state', 'dictionaryCode': contentItem['state_code'], 'object_id': 'name'])

            data['dataStateName'] = ApplicationContextHolder.getBean(DictionaryService.class).readOneByDictionaryTypeCodesCache(['dictionaryTypeCodes': 'noticeDataState,dataState', 'dictionaryCode': contentItem['data_state_code'], 'object_id': 'name'])
